...
class AnimalTestCase(TestCase):
    ...
    def test_get_media_files(self):
        test_file = open("file_synchronizer/tests/test.txt", "w+")
        for x in range(10):
            test_file.write("This is line %d\r\n" % (x + 1))
        test_file.close()
        self.assertEqual(get_files_list(root_media=ROOT).__len__(), 1)
        search_for_files(classes=get_classes(),
                         media_files=get_files_list(root_media=ROOT))
